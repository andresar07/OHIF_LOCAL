...
from key_images_app.tenants.dbsettings import get_tenant_id_from_auth_id
...
class AuthMiddleware:
    ...
    def __call__(self, request):
        ...
        if settings.JWT_AUTH_ENABLED:
            try:
                ...
                if not token:
                    logging.error('Authorization token is mandatory', extra={'tenantId': 'None', 'code': 'VBK001'})
                    raise Exception ('ERROR: Authorization header is mandatory')

                tenant_id = self.validate_auth_token(token)
                if tenant_id:
                    request.tenant_id = tenant_id

            except Exception as e:                
                logging.error(f'Could not validate authorization token. {str(e)}', extra={'tenantId': 'None', 'code': 'VBK002'})
    ...
    def validate_auth_token(self, token):
        ...
        if settings.ONLY_REG_TENANTS_REQUESTS:
            if not auth_tenant:
                logging.error('No tenant information found.', extra={'tenantId': 'None', 'code': 'VBK003'})
                raise Exception ('ERROR: No tenant information found')
            if not tenant_id:
                logging.error(f'Tenant not registered: {str(tenant_id)}', extra={'tenantId': 'None', 'code': 'VBK004'})
