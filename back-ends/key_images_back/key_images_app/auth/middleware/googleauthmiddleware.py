    ...
    def __call__(self, request):
        ...
        if not payload:
            ...
            if settings.JWT_AUTH_ENABLED:
                logging.error('Could not validate authorization token.', extra={'tenantId': 'None', 'code': 'VBK002'})
            ...
            if tenant_id:
                ...
                if settings.ONLY_REG_TENANTS_REQUESTS:
                    logging.error('Could not get tenant_id from payload', extra={'tenantId': 'None', 'code': 'VBK005'})
                    return HttpResponse('Not authorized', status=401)                  
