...
from key_images_app.utils.th_lock import th_lock
...
def get_tenant_db_name(tenant_id):
    # currently, the db key is the same tenant id
    return tenant_id
...
def get_config_from_resource_data(resource_data):
    ...
    if not resource_data['db_name'] or not resource_data['host_name'] or not resource_data['port']:
        logging.error(f"No database resource data found: {str(resource_data)}", extra={'tenantId': 'None', 'code': 'VBK008'})
...
def get_all_resources():
    ...
    try:
        ...
        for resource_id in resource_data:
            ...
            if not config:
                logging.error (f"Can not add config to resource: {str(resource_id)}", extra={'tenantId': 'None', 'code': 'VBK009'})

    except Exception as e:                
        logging.error (f"Couldn't gets resource of type: {str(RESOURCE_TYPE)}", extra={'tenantId': 'None', 'code': 'VBK010'})
        logging.debug (f"Exception: {str(e)}")

    return tmp_db_map

# get resources data for given ids
    ...
    if not resources_ids or len(resources_ids) == 0:
        ...
        for resource_id in resources_ids:            
            ...
            try:
                ...
                if not config:
                    logging.error (f"Resource was not found: {str(resource_id)}", extra={'tenantId': 'None', 'code': 'VBK011'})

            except Exception as e:
                logging.error (f"Couldn't get resource data. Resource Id: {str(resource_id)}", extra={'tenantId': 'None', 'code': 'VBK012'})
                logging.debug (f"Exception: {str(e)}")
    ...
    if not resources_ids or len(resources_ids) == 0:
        ...
        for resource_id in resources_ids:            
            ...
            try:
                    ...
                    for tenant in resource_data['tenants']:
                        ...
                        if auth_id in tmp_auth_tenants_map:
                            ...
                            if tmp_auth_tenants_map[auth_id] != tenant_id:
                                logging.error(f"Authentication id already associated to another tenant. Will be discarded. {str(auth_id)}", extra={'tenantId': 'None', 'code': 'VBK013'})
                                raise Exception(f"ERROR: Authentication id already associated to another tenant.  {str(auth_id)}")
                        ...
                        if tenant_id in tmp_db_tenants_map:
                            if tmp_db_tenants_map[tenant_id] != resource_id:
                                logging.error(f"Tenant Id with more than one resource: {str(tenant_id)}", extra={'tenantId': 'None', 'code': 'VBA014'})
                ...
                logging.error (f"Couldn't get resource tenants. Resource Id: {str(resource_id)}", extra={'tenantId': 'None', 'code': 'VBK006'})
                logging.debug (f"Exception: {str(e)}")
...
def update_db_configs(do_migration=True):
    ...
    # update the tenant resources (db connections kept in db_map)
